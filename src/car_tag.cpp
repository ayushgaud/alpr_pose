...
void Callback(const std_msgs::String::ConstPtr& msg)
{
  ...
  //cars.front().car_marker.type = visualization_msgs::Marker::CUBE;
  	...
  	tf::Pose car_offset;
  	tf::Vector3 t_off(2.0, 0, 0);
  	car_offset.setOrigin(t_off);
  	car_offset.setRotation(tf::Quaternion(0,0,0,1));
  	tf::Pose car = transform * car_offset;
  	cars.front().car_marker.pose.position.x = car.getOrigin().x();
  	cars.front().car_marker.pose.position.y = car.getOrigin().y();
  	cars.front().car_marker.pose.position.z = 0.3;
  	...
  	for (it = cars.begin(); it != cars.end(); ++it)
  	{
        ...
        //std::cout << "Found car: " << it->id << " license plate: "<< it->plate << std::endl;
  			...
  			if(it->confidence  < std::atof(conf.c_str()) - 1)
  			{
  				...
  				car_offset.setRotation(tf::Quaternion(0,0,0,1));
  				tf::Pose car = transform * car_offset;
  				it->car_marker.pose.position.x = car.getOrigin().x(); 
  				it->car_marker.pose.position.y = car.getOrigin().y();
  				it->car_marker.pose.position.z = 0.3;
  				transform.setRotation(transform.getRotation() * tf::createQuaternionFromYaw(M_PI/2) );
  				it->car_marker.pose.orientation.x = transform.getRotation().x();
  				it->car_marker.pose.orientation.y = transform.getRotation().y();
  				it->car_marker.pose.orientation.z = transform.getRotation().z();
  				it->car_marker.pose.orientation.w = transform.getRotation().w();
    		...
    		if(dist > 4.0f)
    		{
    			if(!it_1->empty)
    			{
    				...
    				for(uint k = 0; k < num_markers; k++)
    				{
    					...
    					empty_car_marker.pose.position.z = 0.3;
...
int main(int argc, char **argv)
{
	...
	ros::NodeHandle nh;
	ros::Subscriber sub = nh.subscribe("license_plate", 1, Callback);
	ros::Subscriber pos_sub = nh.subscribe("alpr_pose", 1, PoseCb);
